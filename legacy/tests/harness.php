...
function fixture_load_meta($fixture_dir)
{
    ...
    foreach (array('host', 'rid', 'path', 'url', 'parse_url', 'timezone', 'recorded_at') as $field) {
        if (!isset($meta[$field])) {
...
function fixture_apply_globals($meta)
{
    ...
    $HOST_URL = $meta['url'];
    $URL = strtr($meta['parse_url'], array('${YEAR}' => date('Y')));
    $HOST = $meta['host'];
    $RID = $meta['rid'];
    $TIMEZONE = $meta['timezone'];
    $INFO = isset($meta['info']) ? $meta['info'] : array();
...
function fixture_normalize_contests($contests, $ignore_fields = array())
{
    $normalized = array();
