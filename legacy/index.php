...
foreach (['timezone', 'durationlimit', 'view', 'mode', 'tabs'] as $key) {
    if (isset($_GET[$key])) {
        setcookie($key, $_GET[$key], time() + 365 * 24 * 60 * 60, '/');
    } elseif (isset($_COOKIE[$key])) {
        setcookie($key, $_COOKIE[$key], time() + 365 * 24 * 60 * 60, '/');
    }
}
...
$title_header = '';
switch ($_GET['type']) {
    case 'bsuir-training': $title_header .= ' BSUIR Training.';
        break;
}
...
switch ($view) {
    ...
    default:
        ...
        foreach (['Time', 'Duration', 'Time left', 'Event'] as $s) {
            $table .= "\t\t<td>$s</td>\n";
        }
        $table .= "\t</tr>\n";
        ...
        foreach ($contests as $i => $contest) {
            ...
            $data_contest['date']['start'] = ['time' => date('d.m.y H:i', $start_time), 'url' => $url_start_time];
            $data_contest['date']['end'] = ['time' => date('d.m.y H:i', $end_time), 'url' => $url_end_time];
