...
foreach ($lines as $line) {
    ...
    $abbr_timezones[$abbr] = ['name' => $name, 'timezone' => $timezone];
}
unset($lines);
...
foreach ($resources as $resource) {
    ...
    if ($resource['regexp']) {
        ...
        foreach ($matches as $match) {
            foreach (
                ['date', 'date_start_time', 'date_end_time', 'start_date', 'end_date', 'start_time', 'end_time', 'duration', 'url', 'title', 'key'] as $param
            ) {
                $match[$param] = isset($match[$param]) ? trim(($match[$param])) : '';
            }

            if ($match['date'] == 'Сегодня') {
                $match['date'] = date('Y-m-d', time() + $timezone_offset);
            }
            if ($match['date'] == 'Завтра') {
                $match['date'] = date('Y-m-d', time() + 24 * 60 * 60 + $timezone_offset);
            }

            if ($match['start_date'] && $match['start_time']) {
                $match['start_time'] = $match['start_date'] . ' ' . $match['start_time'];
            }
            if ($match['end_date'] && $match['end_time']) {
                $match['end_time'] = $match['end_date'] . ' ' . $match['end_time'];
            }
...
foreach ($contests as $i => $contest) {
    ...
    if (isset($contest['duration']) && ($contest['duration'] || is_numeric($contest['duration']))) {
        ...
        if (isset($contest['start_time']) && empty($contest['end_time'])) {
            $contest['end_time'] = $contest['start_time'] + $contest['duration'];
        }
        if (empty($contest['start_time']) && isset($contest['end_time'])) {
            $contest['start_time'] = $contest['end_time'] - $contest['duration'];
        }
    }

    if (isset($contest['end_time']) && isset($contest['end_time_shift'])) {
        $contest['end_time'] += strtotime(pop_item($contest, 'end_time_shift'), 0);
    }
    if (isset($contest['start_time']) && isset($contest['start_time_shift'])) {
        $contest['start_time'] += strtotime(pop_item($contest, 'start_time_shift'), 0);
    }
...
foreach ($contests as $i => &$contest) {
    ...
    foreach (
        ['start_time', 'end_time', 'duration', 'url', 'title', 'key', 'standings_url', 'old_key'] as $param
    ) {
...
foreach ($contests as $i => $contest) {
    ...
    if (!DEBUG && !$get_skip_check_time && !$contest_skip_check_time) {
        if ($contest['end_time'] + 31 * 24 * 60 * 60 < time()) {
            continue;
        }
    }
    ...
    if (!isset($contest['key']) || !$contest['key']) {
        $contest['key'] = date('Y', $contest['start_time']) . ' ' . $contest['title'];
    }
    ...
    $unchanged = isset($contest['unchanged']) ? $contest['unchanged'] : [];
