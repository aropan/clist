...
$page = curlexec($url, null, ['no_header' => true]);
...
foreach ($calendar_ids as $calendar_id) {
    ...
    $data = curlexec($url, null, ['http_header' => ["Authorization: $authorization"], 'json_output' => 1]);
    if (!isset($data['items'])) {
        continue;
    }
    foreach ($data['items'] as $item) {
        ...
        } elseif (!$skip_match_by_end && isset($infos[$timestamp])) {
