...
foreach (['', '2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019', '2020'] as $season) {
    foreach (['', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'] as $suffix) {
        if (!$season && $suffix) {
            continue;
        }
        ...
        $normalize_time = function ($time, $year) {
            ...
            if ($month < 4) {
                $time = preg_replace_callback("#(\d+)$#", function ($m) {
                    return $m[1] + 1;
                }, $time);
            }
            return $time;
        ...
        foreach ($matches as $match) {
            ...
            if (preg_match('#[0-9]+\s*-\s*[0-9]+#', $start_time)) {
                ...
                $duration = 2880;
            } elseif (preg_match('#([^-]+)-([^-]+)#', $start_time, $match)) {
                $start_time = $match[1];
                $end_time = $match[2];
                unset($duration);
            } else {
            ...
            if (isset($rtitle) && $round == $rtitle) {
                if (strpos($rdesc, 'before end') !== false) {
                    $end_time = $rtime;
                    unset($duration);
                } elseif (strpos($rdesc, 'before start') !== false) {
                    $start_time = $rtime;
                }
            }
