...
foreach ($URLS as $url) {
    ...
    foreach ($matches as $m) {
        ...
        $round = preg_replace(
            ['#первый#', '#второй#', '#третий#', '#четв[её]+ртый#', '#пятый#', '#шестой#', '#седьмой#', '#восьмой#', '#девятый#'],
            ['1', '2', '3', '4', '5', '6', '7', '8', '9'],
            $round,
        );
        $round = intval($round);
    ...
    if (!preg_match('#<td class="maintext">([^<]*<h2[^>]*>[^\n]*</h2>)?[^<]*<center>[^<]*<table border=1>.*?</table>#s', $page, $match)) {
        continue;
    }
    $table = $match[0];

    if (!preg_match('#<font[^>]*>(?<title>.*?)</font>#', $page, $match)) {
        continue;
    }
