...
$headers = array_map(function ($k, $v) {
    return "$k: $v";
}, array_keys((array) $headers), (array) $headers);
$cookie_file = '/sharedfiles/resource/facebook/cookies.txt';
$curlexec_params = ['http_header' => $headers, 'with_curl' => true, 'cookie_file' => $cookie_file];
...
for (;;) {
    ...
    foreach ($season['season_contests']['nodes'] as $node) {
        ...
        } elseif (isset($node['duration_text']) && preg_match('#^(?P<hours>[0-9]+)\s+hours$#', $node['duration_text'], $match)) {
