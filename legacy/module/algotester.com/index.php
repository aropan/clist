...
$urls = ['https://algotester.com/en/Home/Events'];
...
$scoreboard_urls = ['https://algotester.com/en'];
$seen = [];
foreach ($urls as $url) {
    $page = curlexec($url);
...
foreach ($scoreboard_urls as $scoreboard_url) {
    $page = curlexec($scoreboard_url, null, ['no_logmsg' => true]);
...
foreach ($tournament_urls as $tournament_url) {
    $page = curlexec($tournament_url, null, ['no_logmsg' => true]);
...
foreach ($ids as $tournament_id) {
    ...
    do {
        ...
        $data = curlexec($tournament_url, null, ['no_logmsg' => true, 'http_header' => ['X-Requested-With: XMLHttpRequest'], 'json_output' => 1]);
        if (!isset($data['total'])) {
            break;
        }
        if (isset($data['rows'])) {
