...
class Bot:
    ...
    def subscribe(self, args):
        ...
        def show_subscribed(method):
            ...
            if subscriptions:
                ...
                for subscription in subscriptions:
                    ...
                    message = (
                        f"[{contest.title}]({md_url(contest.actual_url)}) `{contest.pk}` "
        ...
        if created:
            ...
            already_subscriptions = Subscription.objects.filter(
                Q(coder__isnull=False, coder=self.coder)
        ...
        if args.list:
            ...
            statistics_prefetch = Prefetch(
                "statistics_set", queryset=Statistics.objects.filter(contest=contest), to_attr="contest_statistics"
            )
    ...
    def parser(self):
        if not hasattr(self, "parser_"):
            ...
            list_p.add_argument(
                "-lb", "--label", help="Labeling contest chars", nargs=3, default=["", "[running]", "[past]"]
            )
            ...
            subscribe_p.add_argument(
                "names",
                metavar="NAME",
                nargs="*",
                help="Participants names (partial matches allowed). Double quote names with spaces",
            )
            subscribe_p.add_argument("-c", "--contest", help="Contest id, series, name or host")
            subscribe_p.add_argument(
                "-fa", "--first-ac", action="store_true", help="Subscribe to first accepted", default=None
            )
            subscribe_p.add_argument(
                "-nofa", "--no-first-ac", dest="first_ac", action="store_false", help="Unsubscribe to first accepted"
            )
            subscribe_p.add_argument(
                "-tn", "--top-n", type=int, metavar="N", help="Subscribe to top N. Set 0 to unsubscribe top"
            )
            subscribe_p.add_argument(
                "-r", "--remove", action="store_true", help="Remove subscription or specified participants if specified"
            )
