...
function reload_standings(delay) {
  ...
  notify(
    `The page will be reloaded after <span class="countdown" data-timestamp="${timestamp}">${delay / 1000}</span>`,
    { type: "info", duration: delay, escapeHTML: false },
  );
  setTimeout(() => {
    location.reload();
  }, delay);
...
function step_timeline(multiplier = 1, stop = false) {
  ...
  if (
    unfreezing &&
    Math.min(next_percent, CURRENT_PERCENT) < unfreeze_percent() &&
    unfreeze_percent() < Math.max(next_percent, CURRENT_PERCENT)
  ) {
...
function update_timeline_text(percent = null) {
  ...
  var freeze_hidden_percent =
    Math.min(Math.max(contest_time_percentage - Math.max(percent, unfreeze_start), 0), freeze_percent()) * 100;
  ...
  if (unfreeze_is_active(percent)) {
    ...
    $("#timeline-text").text(
      duration_to_text(contest_duration * percent) + " of " + duration_to_text(contest_duration),
    );
  }
...
function clear_data_stat_cell(e) {
  ...
  if (contest_timeline["challenge_score"]) {
    score +=
      parseFloat($(e).attr("data-successful-challenge") || 0) * contest_timeline["challenge_score"]["successful"];
    score +=
      parseFloat($(e).attr("data-unsuccessful-challenge") || 0) * contest_timeline["challenge_score"]["unsuccessful"];
  }
...
function process_problem_cell(element, current_time, unfreeze_index, percentage_filled, callback) {
  ...
  if (problem_submission) {
    ...
    if (penalty && contest_timeline["time_factor"] && times.length in contest_timeline["time_factor"]) {
      var time = parse_factors_time(contest_timeline, penalty, true);
  ...
  for (var index = stat_cells.length - 1; index >= 0; index--) {
    ...
    while ((candidates = $((stat_cell = stat_cells[index])).find(candidates_selector)).length) {
      ...
      candidates.each((_, e) => {
        ...
        penalty = has_empty_penalty
          ? Infinity
          : penalty.split(/[:\s]+/).reduce((r, t) => {
              return r * 60 + parseFloat(t);
            });
      ...
      $(stat_cell)
        .find(".problem-cell.problem-cell-stat")
        .each((_, e) => {
          process_problem_cell(e, current_time, UNFREEZE_SIZE, false, null);
        });
...
function change_freeze_duration(select) {
  ...
  if (value.indexOf(":") !== -1) {
    ...
    freeze_duration = values.reduce((r, t) => {
      return r * 60 + parseFloat(t);
    });
  } else {
...
function clear_starred_unfreezed() {
  ...
  $(".starred.unfreezed").each((_, e) => {
    change_starring.call(e);
  });
  restarred();
}
...
function set_timeline(percent = null, duration = null, scroll_to_element = null) {
  ...
  function process_problem_cell_callback(stat, visible, pvisible, problem_status) {
    ...
    if (toggle_class) {
      if (is_appearancing) {
        ...
        highlight_element(stat, 0, duration / 2, toggle_class, () => stat.attr("data-visible") !== "false", 0.25);
      }
  ...
  $(".problem-cell.problem-cell-stat").each((_, e) => {
    process_problem_cell(e, current_time, unfreeze_index, percentage_filled, process_problem_cell_callback);
  });
  ...
  if (total) {
    $("th.problem-cell[data-problem-key]").each((_, e) => {
      ...
      if (duration && changed_statuses.length) {
        ...
        el.addClass(problem_status_class, duration / 2, () => {
  ...
  rows.find(">.place-cell").each((i, e) => {
    $(e).text($(e).attr("data-text"));
  });
  rows.find(">.gap-cell").each((i, e) => {
    $(e).text($(e).attr("data-text"));
  });
  rows.find(">.first-u-cell >a[data-text]").each((i, e) => {
    $(e).text($(e).attr("data-text"));
  });
  ...
  TRANSFORM_TIMER_ID = setTimeout(() => {
    ...
    rows.each((i, r) => {
      $r = $(r);
      $r.css("transform", $r.attr("data-translate-y"));
    });
...
function highlight_element(
  ...
  setTimeout(() => {
    el.animate({ "background-color": "#d0e3f7" }, duration * duration_ratio, () => {
      el.animate({ "background-color": color }, duration * (2 - duration_ratio), () => {
        el.css("background", "");
...
function parse_factors_time(timeline, penalty, rounding) {
  var timeline_factors = timeline["time_factor"];
  var times = penalty.split(/[:\s]+/);
  ...
  if (rounding) {
    var timeline_rounding = timeline["time_rounding"] ?? "none";
    if (timeline_rounding === "none") {
    } else if (timeline_rounding === "floor-minute") {
      time = Math.floor(time / 60) * 60;
    } else if (timeline_rounding === "floor-second") {
...
function edit_active_switcher_click(event, element) {
  ...
  var dialog = bootbox.dialog({
    size: "small",
    message:
      '<input placeholder="Result" type="text" class="form-control input-sm" value="' +
      score +
      '" name="result" autocomplete="off">' +
      '<input placeholder="Penalty" type="text" class="form-control input-sm" value="' +
      penalty +
      '" name="penalty" autocomplete="off">',
    buttons: {
      cancel: {
        label: "Cancel",
...
function update_score_penalty_result(type) {
  ...
  if (type == "switcher") {
    var edit_btn =
      '<span class="edit-active-switcher" onclick="edit_active_switcher_click(event, this)"><i class="fa-regular fa-pen-to-square"></i></span>';
    ...
    stat.prepend(
      '<div class="swi">' +
        edit_score +
        edit_btn +
        '</div><small class="text-muted"><div>' +
        edit_penalty +
        "</div></small>",
    );
...
function switcher_click(event) {
  ...
  if (stat.attr("data-active-switcher") != "true") {
    ...
    if (contest_timeline["result_switcher_type"] == "next_min") {
      ...
      var problem_results = problem_cells.map((_, e) => {
        return $(e).attr("data-result");
      });
      ...
      penalty = parse_factors_time(contest_timeline, stat.attr("data-penalty"), false) || penalty;
    }
    ...
    if (score) {
      ...
      if (contest_timeline["guaranteed_full_score_factor"]) {
        score = Math.max(
          score,
          stat.attr("data-problem-full-score") * contest_timeline["guaranteed_full_score_factor"],
        );
      }
    } else {
      score = result;
...
function clear_extra_info_timeline() {
  $("table.standings tr > *").classes((c, e) => {
...
function update_trophy_font_size() {
  height =
    Math.min(
      ...$(".handle-cell .trophy-detail")
        .closest(".handle-cell")
        .map((_, e) => $(e).height()),
    ) / 2;
  height = Math.max(14, height);
  $(".handle-cell .trophy-detail").each((_, e) => {
    $(e).css("font-size", height);
  });
}
...
$(() => {
  ...
  $share_timeline.click((e) => {
    ...
    url +=
      (url.indexOf("?") == -1 ? "?" : "&") +
      "timeline=" +
      encodeURIComponent(duration_to_text(contest_duration * CURRENT_PERCENT));
    const keys = ["duration", "step", "delay", "freeze"];
    keys.forEach((k) => {
      el = $("#timeline-" + k);
      url += "&t_" + k + "=" + encodeURIComponent(el.val());
    });
    if (unfreezing) {
      url += "&unfreezing";
    }
...
$(() => {
  ...
  standings_socket.onopen = (e) => {
    standings_socket_opened = true;
  };

  standings_socket.onclose = (e) => {
...
function viewSolution(a, e) {
  ...
  $("#view-solution-modal .modal-content").html(
    '<div class="modal-body"><p><i class="fa fa-spin fa-circle-notch"></i> Loading...</p></div>',
  );
...
function setup_update_contest_time_percentage_by_virtual_start() {
  ...
  if (val) {
    UPDATE_CONTEST_TIME_PERCENTAGE_BY_VIRTUAL_START_TIMER_ID = setInterval(
...
$(() => {
  ...
  function standings_upload_solution(file, problem_cell) {
    ...
    $.ajax({
      ...
      data: form_data,
      beforeSend: () => {
        problem_cell.addClass("uploading");
      },
      success: (data) => {
        notify(data.message, data.status), location.reload();
      },
      error: log_ajax_error_callback,
      complete: () => {
        problem_cell.removeClass("uploading");
      },
    });
  }
});
