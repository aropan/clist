$(() => {
  ...
  $("#theme")
    .editable({
      ...
      showbuttons: false,
      display: function (value, _sourceData, response) {
  ...
  $('#preferences-tab .value [type="checkbox"]')
    .each(function () {
    ...
    .change(function () {
      ...
      $.ajax({
        ...
        error: (xhr, _status, _errorThrown) => {
  ...
  for (i = 1; i < 20; ++i) {
    event_limit_source.push(i.toString());
  }
  $("#event-limit-calendar")
    .editable({
      type: "select",
      showbuttons: false,
  ...
  $.extend(Filter.prototype, {
    render: function () {
      ...
      this.$tpl.find("#select-all-resources").click(() => {
        $("#resources")
          .val($.map(RESOURCES, (r) => r.id))
          .trigger("change");
        return false;
      });

      this.$tpl.find("#deselect-all-resources").click(() => {
        $("#resources").val([]).trigger("change");
        return false;
      });

      this.$tpl.find("#inverse-resources").click(() => {
        $("#resources")
          .val(
            $.grep(
              $.map(RESOURCES, (resource) => resource.id),
              (id) => $.inArray(id, $("#resources").val()) === -1,
            ),
          )
          .trigger("change");
        return false;
      ...
      this.$tpl.find("#inverse-week-days").click(() => {
        $("#week_days")
          .val(
            $.grep(
              $.map(WEEK_DAYS, (week_day) => week_day.id),
              (id) => $.inArray(id, $("#week_days").val()) === -1,
            ),
          )
          .trigger("change");
        return false;
    ...
    value2str: (value) => JSON.stringify(value),
    ...
    input2value: function () {
      ...
      result = {
        ...
        id: parseInt(this.$input.filter('[name="id"]').val(), 10),
        duration: {
          from: parseInt(this.$input.filter('[name="duration-from"]').val(), 10),
          to: parseInt(this.$input.filter('[name="duration-to"]').val(), 10),
        },
        start_time: {
        ...
        resources: $.map(this.$resources.val() || [], (v) => parseInt(v, 10)),
...
<div> \
  ...
  filterEditableOptions = {
    ...
    onblur: "ignore",
    error: (data) => {
      $(".editable-error-block").addClass("alert").addClass("alert-danger").html(data.responseText).show();
    },
  };

  function filterEditableShown(_e, editable) {
  ...
  function filterEditableHidden() {
    var $button = $('[data-id="{0}"]'.format($(this).data("editable").value.id));
    if (
      $(this).data("editable").value.resources.length === 0 &&
    ...
    $.ajax({
      ...
      success: (data) => {
        var $div = $(
          '\
<div> \
    <a href="#" class="filter" data-name="filter" data-value=\'' +
            JSON.stringify(data) +
            '\'></a> \
    <a href="#" data-id="' +
            data.id +
            '" data-action="delete-filter" data-success="$element.remove();" class="action-filter btn btn-default btn-xs"> \
          ...
          `
                <form id="list-form">
                    <div class="form-group">
                        ...
                        <div><input id="list-name" type="text" class="form-control" value="` +
          escape_html(value) +
          `" required></div>
                    ...
                    <div class="form-group">
                        ...
                        <div><select id="list-access-level" class="form-control">` +
          access_level_select_options +
          `</select></div>
                    </div>

                    <div class="form-group">
                        <label class="control-label" for="selectInput">Shared with:</label>
                        <div><select id="list-shared-with" class="form-control" multiple>` +
          shared_with_select_options +
          `</select></div>
                    ...
                    <div class="form-group">
                      ...
                      <input id="list-custom-names" class="hidden" type="checkbox" data-toggle="toggle" data-on="On" data-off="Off" data-onstyle="default" data-offstyle="default" data-size="normal" name="custom_names"` +
          (custom_names ? " checked" : "") +
          `>
                    </div>
                </form>
                <script>
  ...
  function process_calendar() {
    ...
    RESOURCES.forEach((el) => {
      resources_select +=
        ...
        escape_html(el.id) +
        '"' +
        ($.inArray(parseInt(el.id, 10), resources) !== -1 ? " selected" : "") +
        ">" +
        escape_html(el.text) +
        "</option>";
    });
    ...
    EVENT_DESCRIPTIONS.forEach((el) => {
      var has = $.inArray(parseInt(el.id, 10), descriptions) !== -1;
      var option =
        '<option value="' +
        escape_html(el.id) +
        '"' +
    ...
    descriptions.forEach((id) => {
      descriptions_select_before += descriptions_options[id];
    });
    ...
    var form = $(
      `
        ...
        category_select +
        `</select>
              <small class="form-text text-muted">Used to filter contests. Configure <a href="` +
        FILTERS_URL +
        `" target="_blank">here</a>. Optional field</small>
            </div>
            <div class="form-group">
              ...
              <select class="form-control" name="resources" multiple>` +
        resources_select +
        `</select>
              <small class="form-text text-muted">Additional filter for resource. Full list <a href="` +
        RESOURCES_URL +
        `" target="_blank">here</a>. Optional field</small>
            </div>
            <div class="form-group">
              <label class="control-label">Event description</label>
              <select class="form-control" name="descriptions" multiple>` +
        descriptions_select +
        `</select>
  ...
  function process_subscription() {
    ...
    var form = $(
      `
        ...
        ` subscription</h3>
          <form id="subscription_form">
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control" name="top_n" min="1" max="` +
        SUBSCRIPTION_TOP_N_LIMIT +
        `">
            </div>
            <div class="form-group">
              <label class="control-label">Accounts <a href="` +
        ACCOUNTS_URL +
        `" target="_blank">` +
        EXTRA_URL_ICON +
        `</a></label>
              <select class="form-control" name="accounts" multiple></select>
              <small class="form-text text-muted">Limit ` +
        SUBSCRIPTION_N_LIMIT +
        ` accounts</small>
            </div>
            <div class="form-group">
              <label class="control-label">Coders <a href="` +
        CODERS_URL +
        `" target="_blank">` +
        EXTRA_URL_ICON +
        `</a></label>
        ...
        ` coders</small>
            ...
            <div class="form-group">
              <label class="control-label">List <a href="` +
        LISTS_URL +
        `" target="_blank">` +
        EXTRA_URL_ICON +
        `</a></label>
            ...
            <div class="form-group">
              <label class="control-label">Chat <a href="` +
        CHATS_URL +
        `" target="_blank">` +
        EXTRA_URL_ICON +
        `</a></label>
            ...
            <div class="form-group">
              <label class="control-label">Exclude contest</label>
              <select class="form-control" name="exclude_contest"></select>
              <small class="form-text text-muted">Exclude contest from notifications</small>
            </div>
    ...
    var $select_exclude_contest = form.find("select[name=exclude_contest]");
    ...
    bootbox
      .confirm(form, (result) => {
        ...
        $.ajax({
          ...
          data: {
            ...
            exclude_contest: $select_exclude_contest.val(),
          ...
          success: () => window.location.replace(SUBSCRIPTIONS_URL),
          error: (response) => {
      ...
      .on("shown.bs.modal", () => {
        ...
        $select_exclude_contest.select2({
          dropdownParent: dropdown_parent,
          dropdownAutoWidth: true,
          width: "100%",
          theme: "bootstrap",
        ...
        if (data) {
          ...
          if (data.exclude_contest) {
            $select_exclude_contest.select2("trigger", "select", { data: data.exclude_contest });
          }
          if (data.coder_list) {
  ...
  function add_more_filter(div, btn, data) {
    ...
    if (data.more) {
      const a = $('<a class="action-filter btn btn-default btn-xs">');
      copy_attributes(btn, a);
      a.attr("data-page", parseInt(a.attr("data-page"), 10) + 1);
  ...
  function toggle_show_filter(btn, data) {
    ...
    if (previous_shown_filter.length) {
      ...
      const contests_div = $('<div class="shown-filter">');
  ...
  function sentAction() {
    ...
    function queryAction() {
      ...
      $.ajax({
        ...
        data: data,
        success: () => {
          // biome-ignore lint/security/noGlobalEval: eval used here
    ...
    if ($this.attr("data-confirm") === "false") {
      ...
      dialog = bootbox.confirm({
        size: "small",
        message: `${$element.text()}<br/><br/><b>${$this.attr("data-action").replace("-", " ").toTitleCase()}?</b>`,
        callback: (result) => {
          if (!result) {
            return;
          }
