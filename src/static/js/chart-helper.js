function create_chart_config(resource_info, dates, y_field = "new_rating", is_addition = false) {
  if (is_addition) {
    var values = [].concat.apply(
      [],
      $.map(resource_info["data"], function (data, index) {
        return $.map(data, function (val) {
          return val["values"][y_field];
        });
      }),
    );
    ...
    if (min_field === undefined || max_field === undefined || min_field == max_field) {
      notify("Skip " + y_field + " field, min value = " + min_field + ", max value = " + max_field, {
  ...
  var datasets = [].concat.apply(
    [],
    $.map(resource_info["data"], function (data, index) {
      var border_color = get_or_default(datasets_colors[index], "black");
      var dataset = {
        data: $.map(data, function (val) {
          return { x: val["date"], y: is_addition ? val["values"][y_field] : val["new_rating"] };
        }),
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function (tooltipItem, data) {
              ...
              if (hist["old_rating"] || hist["rating_change"]) {
                ...
                if (change > 0) {
                  rating +=
                    ' <span style="font-weight: bold; color: #0f0"><i class="fas fa-angle-up"></i>' +
                    change +
                    "</span>";
                } else if (change < 0) {
                  rating +=
                    ' <span style="font-weight: bold; color: #f00"><i class="fas fa-angle-down"></i>' +
                    -change +
                    "</span>";
                } else {
          ...
          external: function (context) {
            ...
            var left =
              ...
              window.pageXOffset +
              tooltipModel.caretX +
              (tooltipModel.caretX < position.width - tooltipEl.offsetWidth - 10 ? 0 : -tooltipEl.offsetWidth);
            tooltipEl.style.left = left + "px";
            var top =
              position.top +
              window.pageYOffset +
              tooltipModel.caretY +
              (tooltipModel.caretY < position.height - tooltipEl.offsetHeight - 10 ? 0 : -tooltipEl.offsetHeight);
...
function add_selection_chart_range(canvas_selector, chart, with_close_chart = false, range_selection = undefined) {
  ...
  function set_chart_range(evt) {
    ...
    if (dragX && draged) {
      ...
      const start_value =
        ((selectionRect.startY - y_axis.top) / y_axis.height) * (y_axis.min - y_axis.max) + y_axis.max;
