...
String.prototype.toTitleCase = function () {
  ...
  return this.replace(/[A-Za-z0-9\u00C0-\u00FF]+[^\s-]*/g, (match, index, title) => {
    if (
      index > 0 &&
...
$(() => {
  $(".sortable-column").each(function () {
    ...
    if (sort_column == column) {
      ...
      $(this).append(
        `<a href="` + disable_url + `"><i class="sortable-column-order fas fa-chevron-` + order + `"></i></a>`,
      );
    }
...
function inline_button() {
  $(".reset-timing-statistic")
    .removeClass("reset-timing-statistic")
    .click(function (e) {
      e.preventDefault();
      var btn = $(this);
      $.post("/standings/action/", {
        action: "reset_contest_statistic_timing",
        cid: btn.attr("data-contest-id"),
      })
        .done((data) => {
...
$(() => {
  $(".copy-to-clipboard:not([onclick])").click(function (event) {
    copyElementToClipboard(event, this);
  });
});
...
function note_action() {
  ...
  var loading_timeout_id = setTimeout(() => {
...
function configure_pagination(paginate_on_scroll = true) {
  ...
  $(document)
    .off("ajaxError.endlessPaginate")
    .on("ajaxError.endlessPaginate", (_event, xhr, settings) => {
      log_ajax_error(xhr);
    });
}
...
function checkbox_mouseover_toggle() {
  $('input.mouseover-toggle[type="checkbox"]')
    .removeClass("mouseover-toggle")
    .mouseover(function () {
      if (mouse_is_down) {
...
function clear_url_parameters() {
  ...
  if (url.searchParams.has("search") || url.searchParams.has("sort_order")) {
    ...
    if (sort_orders.length > 1) {
      update_urls_params({
        sort_order: sort_orders[sort_orders.length - 1],
      });
    }
    if (to_remove) {
      to_remove = [...to_remove].reduce((obj, key) => {
        obj[key] = undefined;
        return obj;
      }, {});
...
function update_table_tr_hover() {
  $("table.table-hover tr:not(.sticky-hovered):has(td)")
    .addClass("sticky-hovered")
    .hover(
      function () {
        var $el = $(this);
