...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if self.config["stop_email"]["n_failed"] >= self.N_STOP_EMAIL_FAILED_LIMIT and now() - self.config[
            "stop_email"
        ]["failed_time"] < timedelta(hours=2):
            stop_email = True
        ...
        if coders:
            qs = qs.filter(
                Q(periodical_notification__coder__username__in=coders) | Q(subscription__coder__username__in=coders)
            )
