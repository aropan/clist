...
class AvailableForUpdateResourceManager(BaseManager):
    def get_queryset(self):
        ret = super().get_queryset()
        with_updating = (
            Q(has_rating_history=True)
            | Q(has_country_rating=True)
            | Q(has_problem_rating=True)
        ...
        parse_statistic_in_progress = EventLog.objects.filter(
            ...
            modified__gt=timezone_now() - timedelta(hours=1),
...
class Resource(BaseModel):
    ...
    RATING_FIELDS = (
        ...
        "NewRating",
        "Performance",
        "predicted_old_rating",
        "predicted_new_rating",
        "predicted_rating_perf",
        "predicted_raw_rating",
        "rating_prediction_old_rating",
        "rating_prediction_new_rating",
        "rating_prediction_rating_perf",
    ...
    def save(self, *args, **kwargs):
        if self.color is None:
            ...
            opt = 0, 0
            for val in values:
                delta, middle, prv = val - prv, (val + prv) * 0.5, val
                opt = max(opt, (delta, middle))
            self.color = rgb_to_color(*hls_to_rgb(opt[1] % 1, 0.6, 0.5))
    ...
    def with_single_account(self, account=None):
        return not self.has_multi_account or (account is not None and account.rating is not None)
...
class OngoingContestManager(SignificantContestManager):
    def get_queryset(self):
        ret = super().get_queryset()
        ret = Contest.objects.annotate(duration_time=Epoch(F("end_time") - F("start_time"))).filter(
...
class Contest(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_rated is None:
            is_rated = self.related_id is None and (
                "new_rating" in fields or "rating_change" in fields or "_rating_data" in fields
            )
            if is_rated:
                self.is_rated = True
    ...
    def is_finalized(self):
        return self.is_over() and not self.has_hidden_results and not self.info.get("_timing_statistic_delta_seconds")
    ...
    def get(value) -> Optional["Contest"]:
        ...
        if isinstance(value, str):
            qs = Contest.objects.filter(
                Q(title=value)
                | Q(resource__host=value)
                | Q(host=value)
                | Q(series__name=value)
                | Q(series__slug=slug(value))
            )
...
class Problem(BaseModel):
    contest = models.ForeignKey(
        Contest, null=True, blank=True, on_delete=models.CASCADE, related_name="individual_problem_set"
    )
