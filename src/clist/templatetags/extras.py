...
def standings_statistic_problem(context, scoreformat_cache=None):
    ...
    if has_alternative_result:
        ...
        html_parts.append(
            f'<span class="detail-alternative-result hidden">{html.escape(str(alternative_result))}</span>'
        )
    ...
    if stat.get("icon"):
        ...
        for i, subscore in enumerate(stat["subscores"]):
            ...
            subscore_title = (
                f' title="{html.escape(str(subscore_verdict))}" data-toggle="tooltip"' if subscore_verdict else ""
            )
            subscore_class = "acc" if subscore.get("result") else "rej"
            html_parts.append(
                f'<span{subscore_title} class="{subscore_class}">{html.escape(str(subscore.get("status", "")))}</span>'
            )
        html_parts.append("</span>")
    ...
    if stat.get("language") and languages:
        ...
        html_parts.append(
            f'<small class="{lang_class}"><div class="language">{html.escape(str(stat["language"]))}</div></small>'
        )
    ...
    if stat.get("verdict") and verdicts:
        ...
        html_parts.append(
            f'<small class="{verdict_class}"><div class="verdict">{html.escape(str(stat["verdict"]))}</div></small>'
        )
