...
class ProblemResource(BaseModelResource):
    ...
    contest_ids = fields.ListField("contest_ids", null=True, help_text="A list of data. Ex: {'abc', 26.73, 8}")
    ...
    def get_object_list(self, request):
        ...
        problems = problems.annotate(
            solved=ExpressionWrapper(Q(system_solved=True) | Q(user_solved=True), output_field=models.BooleanField())
        )
        problems = problems.annotate(
            reject=ExpressionWrapper(Q(system_reject=True) | Q(user_reject=True), output_field=models.BooleanField())
        )
