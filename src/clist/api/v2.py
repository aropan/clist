...
class StatisticsResource(BaseModelResource):
    ...
    class Meta(BaseModelResource.Meta):
        ...
        filtering = {
            ...
            "account_id": ["exact", "in"],
            "coder_id": ["exact"],
            "place": ["exact", "gt", "lt", "gte", "lte", "isnull"],
            "new_rating": ["isnull"],
...
def use_in_me_only(bundle, *args, **kwargs):
    ...
    url = reverse(
        f"clist:api:{api_version}:api_dispatch_me", kwargs={"api_name": api_version, "resource_name": "coder"}
    )
...
def use_in_detail_only(bundle, *args, **kwargs):
    ...
    api_version = get_api_version(bundle)
    url = reverse(
        f"clist:api:{api_version}:api_dispatch_list", kwargs={"api_name": api_version, "resource_name": "coder"}
    )
