...
def resources_country_ratings(request, template="resources_country_ratings.html"):
    ...
    resources = resources.prefetch_related(Prefetch("countryaccount_set", queryset=country_accounts))
...
def problems(request, template="problems.html"):
    ...
    if request.user.is_authenticated:
        ...
        problem_rating_accounts = coder.account_set.filter(
            rating__isnull=False, resource__has_problem_rating=True
    ...
    if chart_field == "rating":
        n_bins, step = (
            ...
            if selected_resource and selected_resource.has_rating_history
            else (None, None)
        )
    ...
    status_select = {
        ...
        "data": [
            ...
            for name in ["allsolved", "allreject"]
        ...
        + [
            ...
            for name in ["todo", "solved", "reject"]
            for enable in [True, False]
        ]
        + [
            {
