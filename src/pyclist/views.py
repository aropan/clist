...
def update_context_by_source(request, context):
    ...
    for selection in selections:
        ...
        field_select = create_field_to_select(
            values=values, name=selection, multiply=True, ajax=True, groupby=bool(x_axis)
        )
...
@superuser_required
@page_templates((("charts_paging.html", "entities_paging"),))
...
def charts(request, template="charts.html"):
    ...
    for model in app_models:
        ...
        model_fields = models.setdefault(
            source,
            {
                "name": source,
                "model": model,
