...
def renaming_check(account, contest_keys, fields, contest_addition_update):
    ...
    if max_counter_val < threshold_val:
        LOG.warning(
            "Failed renaming %s, max_key = %s, max_val = %d, threshold = %d, n_counters = %d",
            account,
            max_counter_key,
            max_counter_val,
            threshold_val,
...
def fill_missed_ranks(account, contest_keys, fields, contest_addition_update):
    ...
    for contest_key in tqdm.tqdm(contest_keys, desc="fill missed ranks", total=len(contest_keys)):
        ...
        if not contests:
            qs = base_contests.annotate(
                has_skip=Exists("statistics", filter=Q(place_as_int=rank, skip_in_stats=True, account=account))
            )
...
def account_update_contest_additions(
    ...
    while contest_keys:
        ...
        if contest_keys:
            ...
            LOG.warning(
                "Not found %d contests for %s = %s%s",
                len(out_contests),
                account,
                out_contests[:5],
                "..." if len(out_contests) > 5 else "",
            )
        break
...
def update_stage(self):
    ...
    if parsed_statistic:
        call_command("parse_statistic", contest_id=stage.pk, without_set_coder_problems=True, without_stage=True)
    ...
    for idx, contest in enumerate(tqdm.tqdm(contests, desc=f"getting contests for stage {stage}"), start=1):
        info = {
            ...
            "name": contest.title,
            "url": reverse(
                "ranking:standings", kwargs={"title_slug": slug(contest.title), "contest_id": str(contest.pk)}
            ),
        }
    ...
    statistics = Statistics.objects.select_related("account", "account__duplicate").prefetch_related("account__coders")
