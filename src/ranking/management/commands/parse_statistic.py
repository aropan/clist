...
from ranking.models import Account, AccountRenaming, Module, Stage, Statistics, StatisticsLog
    ...
    def parse_statistic(
        ...
        if contest_id:
            ...
            if with_check:
                if previous_days is not None:
                    ...
                    contests_query = (
                        Q(pk__in=parsed_contests) | Q(pk__in=in_range_contests) | Q(pk__in=inherit_contests)
                    )
        ...
        for contest in progress_bar:
            ...
            if resource.has_upsolving and not with_stats:
                self.logger.warning(
                    f"Skip parse statistic contest = {contest} because run without stats and resource has upsolving"
                )
                continue
            ...
            try:
                ...
                is_archive_contest = (
                    max(contest.created, contest.end_time) + timedelta(days=30) < now and contest.parsed_time
                )
                ...
                with_versus_subscription = not without_subscriptions and get_item(contest.info, "_has_versus.enable")
                with_submissions_statistics_log = resource.has_submissions_statistics_log
                prefetch_subscribed_coders = Prefetch(
                    "coders", queryset=Coder.objects.filter(n_subscribers__gt=0), to_attr="subscribed_coders"
                )
                subscription_top_n = None
                subscription_first_ac = None
                base_subscriptions_filter = Q()
                if with_subscription:
                    base_subscriptions_filter = (
                        (Q(resource__isnull=True) | Q(resource=resource))
                        & (Q(contest__isnull=True) | Q(contest=contest))
                        & (~Q(exclude_contest=contest))
                    )
                    subscriptions = Subscription.for_statistics.filter(base_subscriptions_filter)
                ...
                with transaction.atomic():
                    ...
                    standings_hidden_fields_mapping = {}
                    ...
                    if result or specific_users:
                        ...
                        for r in result.values():
                            ...
                            def update_problems_info():
                                ...
                                default_full_score = contest.info.get(
                                    "default_problem_full_score"
                                ...
                                for k, v in problems.items():
                                    ...
                                    if (
                                        contest.info.get("with_last_submit_time")
                                        and scored
                                        and ("_last_submit_time" not in r or r["_last_submit_time"] < v["time"])
                                    ):
                                    ...
                                    if ac and v.get("time"):
                                        if (
                                            contest.info.get("with_time_from_timeline")
                                            and "time_in_seconds" not in v
                                            and contest_timeline
                                        ):
                        ...
                        if (
                            len(standings_kinds) == 1
                            and (not contest.standings_kind or contest.standings_kind in guess_standings_kinds)
                            and contest.standings_kind != (standings_kind := standings_kinds.pop())
                        ):
                        ...
                        if contest.set_matched_coders_to_members:
                            ...
                            matched_coders = matched_coders.values(
                                "name", "statistic_id", "coder__username", "coder__country"
                            )
                            matched_coders = {
                        ...
                        for r in tqdm(results, desc="update results"):
                            ...
                            def update_account_time():
                                if (
                                    contest.info.get("_no_update_account_time")
                                    or skip_result
                                    or contest.end_time > now
                                    or specific_users
                                ):
                                    return
                                ...
                                if (
                                    ...
                                    if division not in user_info_has_rating:
                                        ...
                                        try:
                                            ...
                                            field = (
                                                user_info.get("contest_addition_update_by") or params.get("by") or "key"
                                            )
                                            addition_update = (
                                                user_info.get("contest_addition_update") or params.get("update") or {}
                                            )
                            ...
                            def update_account_info():
                                ...
                                if r.get(field_update_name):
                                    ...
                                    if (
                                        not no_update_name
                                        and account.name != r[field_update_name]
                                        and account.key != r[field_update_name]
                                    ):
                                ...
                                if contest_addition_update:
                                    account_update_contest_additions(
                                        account, contest_addition_update, timedelta(days=31) if with_check else None
                                    )
                                ...
                                if (last_submission := r.pop("_last_submission", None)) and (
                                    not account.last_submission or last_submission > account.last_submission
                                ):
                            ...
                            def update_stat_info():
                                ...
                                if medals and contest.end_time < now and not contest.has_hidden_results:
                                    ...
                                    if "place" in r and (not medals_divisions or r.get("division") in medals_divisions):
                                        ...
                                        if member in contest.info.get("standings", {}).get("medals_skip", []) or r.get(
                                            "_skip_medal"
                                        ):
                                            ...
                                            for medal in medals:
                                                if ("count" in medal and place <= medal["count"]) or (
                                                    "score" in medal and r.get("solving", 0) >= medal["score"]
                                                ):
                                                    r[k] = medal["name"]
                                                    if "field" in medal:
                                                        field = medal["field"]
                            ...
                            def get_addition():
                                ...
                                try_calculate_time = contest.calculate_time or (
                                    ...
                                    and not get_item(resource, "info.parse.no_calculate_time")
                                    and contest.full_duration < resource.module.long_contest_idle
                                    and "penalty" in fields_set
                                    and is_fast_reparse
                                )
                            ...
                            def update_submissions(statistic, result_submissions):
                                ...
                                for result_submission in result_submissions:
                                    ...
                                    problem = Problem.cached_get(
                                        contest=contest, short=result_submission["problem_short"]
                                    )
                                    ...
                                    if "testing" in result_submission and submission_created:
                                        ...
                                        for testing in result_submission["testing"]:
                                            ...
                                            if (
                                                not verdict.solved
                                                and test_number is not None
                                                and "failed_test" not in defaults
                                                and (
                                    ...
                                    if statistic_problem and submission.current_result == statistic_problem.get(
                                        "result"
                                    ):
                                        fields_values = (
                                        ...
                                        if contest.calculate_time and not is_solved(submission.current_result):
                                            time = time_in_seconds_format(
                                                contest_timeline, int(submission.contest_time.total_seconds()), num=2
                                            )
                            ...
                            def process_subscriptions(statistic, updates):
                                ...
                                if (
                                    not account.n_subscribers
                                    and not subscribed_coders
                                    and not with_top_n
                                    and not with_first_ac
                                ):
                                    return
                            ...
                            def process_upsolving_subscriptions(submissions, condition, subscription_model, cache):
                                ...
                                if subscribed_coders:
                                    subscriptions_filter |= Q(coders__in=subscribed_coders)
                                subscriptions_filter = subscriptions_filter & base_subscriptions_filter
                                upsolving_subscriptions = subscription_model.filter(subscriptions_filter)
                            ...
                            def update_submissions_statistics_log(statistic, submissions):
                                if not with_submissions_statistics_log:
                                    return
                                for submission in reversed(submissions):
                                    submission_id = submission.get("id", submission.get("submission_id"))
                                    submission_time = submission.get("time", submission.get("submission_time"))
                                    log_type = submission.pop("type", StatisticsLog.LogType.SUBMISSION)
                                    _, statistics_log_created = StatisticsLog.objects.update_or_create(
                                        resource=resource,
                                        contest=contest,
                                        account=account,
                                        statistic=statistic,
                                    ...
                                    contest_log_counter["statistics_log_total"] += 1
                                    contest_log_counter["statistics_log_created"] += statistics_log_created
                            ...
                            updates = update_after_update_or_create(
                                statistic, statistic_created, addition, try_calculate_time
                            )
                            ...
                            process_upsolving_subscriptions(
                                result_upsolving_submissions,
                                with_upsolving_subscription,
                                Subscription.for_upsolving,
                                upsolving_cache,
                            )
                            process_upsolving_subscriptions(
                                result_upsolving_submissions,
                                with_versus_subscription,
                                Subscription.for_statistics,
                                upsolving_cache,
                            )

                            update_submissions_statistics_log(statistic, result_submissions)
                            upsolving_submissions = [submission["info"] for submission in result_upsolving_submissions]
                            update_submissions_statistics_log(statistic, upsolving_submissions)
                        ...
                        if not specific_users:
                            ...
                            while fields_preceding:
                                ...
                                for v in fields_preceding.values():
                                    v.discard(k)

                            if canonize(fields) != canonize(contest.info.get("fields")):
                                contest.info["fields"] = fields

                            standings_hidden_fields = [
                                standings_hidden_fields_mapping.get(f, f) for f in standings_hidden_fields
                            ]
                            ...
                            if standings_problems is not None:
                                ...
                                if "division" in problems:
                                    problems = sum_lists(problems["division"].values())
                    ...
                    if not specific_users:
                        timing_delta = None
                        if contest.full_duration < resource.module.long_contest_idle or standings.get(
                            "force_timing_statistic_delta"
                        ):
                ...
                if not contest.statistics_update_required and contest.pk:
                    ...
                    if (
                        resource.has_problem_rating
                        and contest.is_finalized()
                        and not without_calculate_problem_rating
                    ...
                    if contest.is_finalized():
                        ...
                        for orig_contest in related_contests:
                            ...
                            if not orig_contest.with_medals or related_contest.with_medals:
                                continue
                            self.logger.info(
                                f"Inherit medals to related contest = {related_contest}, from contest = {orig_contest}"
                            )
                    ...
                    if contest.is_finalized() and not without_set_coder_problems:
                        if specific_users:
                            ...
                            if users_coders:
                                self.logger.info(f"Set coder problems for contest = {contest}, coders = {users_coders}")
