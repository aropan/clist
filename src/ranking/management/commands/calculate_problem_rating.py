    ...
    def handle(self, *_, **options):
        ...
        if resources.count() == 1 and contests_count > 5:
            resource_event_log = EventLog.objects.create(
                name="calculate_problem_rating", related=contests.first().resource, status=EventStatus.IN_PROGRESS
            )
        else:
        ...
        for contest in tqdm.tqdm(contests, total=contests_count, desc="contests"):
            ...
            ignore_missing_account = args.ignore_missing_account or problem_rating_info.get("ignore_missing_account")
            ...
            event_log = EventLog.objects.create(
                name="calculate_problem_rating", related=contest, status=EventStatus.IN_PROGRESS
            )
            ...
            for problem in contest.problem_set.all():
                if problem.rating is None:
                    ...
                    break
            if not empty_problem_rating and not args.force and contest.problem_rating_hash == problem_rating_hash:
                n_skip_hash += 1
            ...
            for current_contest, current_statistics in problems_contests.items():
                ...
                for stat in tqdm.tqdm(
                    current_statistics, total=current_statistics.count(), desc="accounts_to_get_n_contests"
                ):
                    if is_skip(current_contest, stat):
                        continue
        ...
        self.logger.info(
            f"done = {n_done}, skip hash = {n_skip_hash}, skip missing = {n_skip_missing}"
