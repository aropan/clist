...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--rank-update-delay", help="time delay after rank last update time", default="5 days")
        parser.add_argument("--contest-update-delay", help="time delay after rating last update time", default="1 days")
    ...
    def handle(self, *args, **options):
        ...
        if not args.without_delay and not args.force and not args.resources:
            ...
            need_update = Q(country_rank_update_time__isnull=True) | (
            ...
            long_update = Q(country_rank_update_time__isnull=True) | Q(
                country_rank_update_time__lt=now - rank_update_delay
            )
            ...
            with stream_event_log(event_log, self.logger):
                with measure_time("country_accounts", logger=self.logger):
                    ...
                    with suppress_db_logging_context():
                        country_accounts = CountryAccount.objects.bulk_create(
                            country_accounts,
                ...
                def update_rating_fields():
                    ...
                    with suppress_db_logging_context():
                        CountryAccount.objects.bulk_update(
                            country_accounts_update, ["rating", "n_rating_accounts", "raw_rating"]
                        )
                ...
                resource.countryaccount_set.all().update(
                    ...
                    n_other_medals=None,
                    n_first_places=None,
                    n_second_places=None,
                    n_third_places=None,
                    n_top_ten_places=None,
                )
