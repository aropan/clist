    ...
    def handle(self, *args, **options):
        ...
        if args.timing_delay:
            ...
            contests = contests.filter(
                Q(link_statistic_timing__lt=now - timing_delay) | Q(link_statistic_timing__isnull=True)
            )
        ...
        def link_contest(contest):
            ...
            def get_related_account(related_stat):
                ...
                account, created = Account.objects.update_or_create(
                    resource=resource, key=related_account.key, defaults=defaults
                )
                if created:
            ...
            for stat in tqdm.tqdm(contest_stats.iterator(), desc=f"statistics {contest}"):
                ...
                for related_stat in mapped_related_stats:
                    ...
                    ok = stat.place_as_int == related_stat.place_as_int and len(mapping_stats[stat.place_as_int]) == 1
                    if not ok:
                        failed = True
                        self.logger.warning(
                            f"Cannot find related statistic for {stat} #{stat.place_as_int}"
