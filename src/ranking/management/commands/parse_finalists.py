...
from ranking.models import Account, Finalist, FinalistResourceInfo
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for finalist_info in tqdm.tqdm(finalists):
            ...
            max_accounts_size = max(max_accounts_size, len(accounts))
            has_country = has_country or any(account.country for account in accounts)
            ...
            for resource in resources:
                ...
                # if not force:
                #     if (not with_update or resource_info.updated
                #         and last_modified < resource_info.updated + update_delay):
                #         continue
                ...
                accounts = rating_accounts.filter(accounts_filter).union(rating_accounts.filter(coders_filter))
                ratings_data = accounts.values("rating", "key")
                if not ratings_data:
                    continue
