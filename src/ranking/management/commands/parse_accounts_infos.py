...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-l", "--limit", default=None, type=int, help="limit users for one resource (default is 1000)"
        )
                ...
                if args.query:
                    ...
                    if resource_info.get("force_on_new_year") or args.update_new_year:
                        ...
                        if days <= 14 or args.update_new_year:
                            ...
                            rating95 = accounts.aggregate(rating95=Percentile("rating", 0.95))["rating95"]
                ...
                with tqdm(total=len(accounts), desc=f"getting {resource.host} (total = {total})") as pbar:
                    ...
                    for account, data in zip(accounts, infos):
                        ...
                        with transaction.atomic():
                            ...
                            if coders:
                                qs = Coder.objects.filter(account__resource=resource, account__key__in=coders).exclude(
                                    account=account
                                )
                                for c in qs:
                            ...
                            do_upsolve = resource.has_upsolving and (
                                (account.has_coders and not account.info.get("is_team"))
                                or (upsolving_subscriptions := get_subscriptions_with_upsolving(resource, account))
                            )
                            upsolve_delay = None
                            if do_upsolve:
