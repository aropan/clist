...
class Statistic(BaseModule):
    def get_standings(self, users=None, statistics=None, **kwargs):
        ...
        for r in table:
            ...
            name = r["Problem"]
            column = name.column
            title = column.node.xpath(".//*[@class='plist__name']")[0]
            name = title.xpath("text()")[0]
            name = re.sub(r"\s+", " ", name).strip()
            problem["name"] = name
            if not re.match(r"^[A-Z][0-9]*\.", name):
                with_letter = False

            href = title.xpath('.//a[starts-with(@href,"/p/")]/@href')
            if href:
                url = urllib.parse.urljoin(REQ.last_url, href[0])
                problem["url"] = url
                problem["code"] = url.strip("/").split("/")[-1]

            if (difficulty := r.get("Difficulty")) and difficulty.value:
                problem["difficulty"] = slug(difficulty.value)

            problem["writers"] = column.node.xpath('.//a[contains(@class,"handle")]/text()')
        ...
        def has_mark(v, mark: str) -> bool:
            return bool(
                v.column.node.xpath(f'.//img[contains(@src,"/{mark}.svg")]')
                or v.column.node.xpath(f'.//*[contains(@href,"#{mark}")]')
            )
                ...
                for k, v in r.items():
                    if k == "#":
                            ...
                            if "rating" in field.attrs.get("class", "").split():
                                ...
                                if cs:
                                    ...
                                    if "fa-angle-double-down" in cs or "text-muted" in cs:
                                        row["rating_change"] = -int(field.value)
                                        break
                            trendup = has_mark(field, "trendup") or has_mark(field, "trending-up")
                            trenddown = has_mark(field, "trenddown") or has_mark(field, "trending-down")
                        ...
                        if (
                            ...
                            is_accepted = has_mark(v, "check") or is_first_ac
                            is_hidden = has_mark(v, "question")
                            is_failed = has_mark(v, "cross")
                            if v.value or is_hidden or is_failed:
                                ...
                                elif not is_accepted and v.value:
