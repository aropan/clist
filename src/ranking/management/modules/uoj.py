...
class Statistic(BaseModule):
    def get_standings(self, users=None, statistics=None, **kwargs):
        ...
        def fetch_problem(problem: tuple[int, int]):
            ...
            for url in (
                ...
                try:
                    ...
                    match = re.search(rf"<h1[^>]*>\s*{prefix}.(.*?)</h1>", problem_page, re.DOTALL)
                    if not match:
                        continue
                    name = match.group(1)
        ...
        for standings_row in standings:
            ...
            scoring_values = scoring.items() if isinstance(scoring, dict) else enumerate(scoring)
            for k, scoring_value in scoring_values:
                score, time, submission_id, *scoring_value = map(as_number, scoring_value)
                short = problems_infos[int(k)]["short"]
                problem = problems.setdefault(short, {})
                problem["result"] = as_number(score)
                if scoring_value and len(scoring_value) == 3:
