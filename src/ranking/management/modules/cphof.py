...
class Statistic(BaseModule):
    ...
    def _parse_profile_page(url, safe=False):
        ...
        match = re.search(
            r'>country:</[^>]*>(?:\s*<[^>]*>)*\s*<a[^>]*href="[^"]*/country/(?P<country>[^"]*)"', page, re.IGNORECASE
        )
        if match:
    ...
    def get_users_infos(users, resource, accounts, pbar=None):
        ...
        def link_accounts(info, user, cphof_resource, cphof_account):
            ...
            if len(coders_set) - n_virtual > 1:
                raise ExceptionParseAccounts(
                    f"Too many coders for {user}: {coders_set} ({n_virtual} of {len(coders_set)} virtual)"
                )
