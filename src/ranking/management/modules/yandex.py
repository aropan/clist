    ...
    def _get_upsolving_submissions(self, contest, problems_info, names_result):
        ...
        for contest_key, contest_url in (
            ...
            for submission in self._get_all_submissions(contest_key):
                ...
                if problem and submission_result_value >= problem_result_value and (
                    'submission_time' not in problem or
                    submission_info['submission_time'] < problem['submission_time']
                ):
            ...
            while True:
                ...
                for r in table:
                    ...
                    for k, v in list(r.items()):
                        if 'table__cell_role_result' in v.attrs['class']:
                            ...
                            if n.xpath('img[contains(@class,"image_type_success")]'):
                                ...
                                res = res.replace(',', '')
                                if not res.startswith('?') and as_number(res, force=True) is None:
                                    problems.pop(letter)
                                    continue
                            p['result'] = res
                            if ' ' in v.value:
                    ...
                    if name2logins:
                        ...
                        self.resource.has_standings_renamed_account and
