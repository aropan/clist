...
class Statistic(BaseModule):
    ...
    def get_standings(self, users=None, statistics=None, **kwargs):
        ...
        def get_problem_full_score(info):
            try:
                ...
                if problem_key in info_problems and "full_score" in info_problems[problem_key]:
                    ...
                    LOG.warning(
                        f"Failed to get full score for {problem_key}, using cached value = {info['full_score']}"
                    )
                else:
                    raise e
        ...
        for row in data["StandingsData"]:
            ...
            r["solving"] = total_result["Score"] / 100.0
        ...
        if has_rated and not has_new_rating and self.end_time < now() < self.end_time + timedelta(hours=3):
    ...
    def update_submissions(account, resource, **kwargs):
        ...
        while not stop:
            ...
            for submission in submissions:
                ...
                problem_score = problem.get("result", 0)
                if (problem_score, problem.get("submission_time", 0)) >= (
                    problem_info["result"],
                    problem_info["submission_time"],
                ):
                    continue
                ...
                if problem_score < eps and problem_info["result"] < eps:
                    ...
                    problem_score == problem_info["result"]
                    and "submission_time" in problem
                    and problem["submission_time"] < problem_info["submission_time"]
                ):
                    continue
