    ...
    def get_standings(self, users=None, statistics=None, **kwargs):
        ...
        def get_leaderboard(url, column="", value="", limit=None):
            ...
            if is_clash:
                ...
                post = (
                    '{"operationName":"GeneralLeaderboard","variables":{"handle":"'
                    + handle
                    + '","limit":'
                    + str(limit)
            ...
            if is_escape:
                ...
                if len(data["users"]) >= 1000:
                    ...
                    if not n_rows or max(n_rows) >= 1000:
                        ...
                        for data in tqdm.tqdm(
                            executor.map(fetch_data, languages), total=len(languages), desc="languages"
                        ):
                            process_data(data)

                        fetch_data = partial(get_leaderboard, url, "COUNTRY")
                        for data in tqdm.tqdm(
                            executor.map(fetch_data, countries), total=len(countries), desc="countries"
                        ):
                            process_data(data)
                ...
                for r in result.values():
                    if "percentage" not in r:
                        continue
                    if r["percentage"] < 100:
                        if (score_percentage := r.get("_score_percentage")) and (
                    ...
                    if (
                        ...
                        and score_history[-1]["timestamp"] > self.end_time.timestamp()
                    ):
                        continue
