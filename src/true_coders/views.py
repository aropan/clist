...
from clist.templatetags.extras import (
    ...
    asfloat,
    format_time,
    get_division_problems,
    get_item,
    get_problem_short,
    get_timezones,
    has_update_statistics_permission,
    is_rating_prediction_field,
    is_yes,
    query_transform,
    quote_url,
    relative_url,
...
def coders(request, template="coders.html"):
    ...
    if filtered_stats:
        coders = coders.annotate(
            has_contest_account=Exists(filtered_stats["accounts"].filter(coders=OuterRef("id")))
...
def get_ratings_data(
    ...
    qs = [
        ...
        for stat in qs
        if (
            stat["resource"] in resources
            and (stat["new_rating"] or stat.get("addition___rating_data"))
            and not stat["is_unrated"]
        )
    ]
    ...
    for stat in qs:
        ...
        if stat.get("addition___rating_data"):
            data = resource.plugin.Statistic.get_rating_history(
                stat["addition___rating_data"], stat, resource, date_from=date_from, date_to=date_to
            )
            if data:
...
def change(request):
    ...
    if name == "theme":
        ...
        try:
            ...
            if name == "add-calendar":
                calendar = Calendar.objects.create(
                    coder=coder, name=value, category=category, resources=resources, descriptions=descriptions
                )
            ...
            return HttpResponseBadRequest("Something went wrong")
        return HttpResponse(
            json.dumps({"id": calendar.pk, "name": calendar.name, "filter": calendar.category}),
        ...
        exclude_contest_id = request.POST.get("exclude_contest") or None
        ...
        if n_choosen + n_addition < 1:
            return HttpResponseBadRequest(
                "choose at least one of accounts/coders or coder list or coder chat or first accepted/top n"
            )
        if coder_list_id:
        ...
        if name == "add-subscription":
            subscription = Subscription.objects.create(
                ...
                exclude_contest_id=exclude_contest_id,
            ...
            subscription.exclude_contest_id = exclude_contest_id
        ...
        if username != user.username:
            return HttpResponseBadRequest(
                f"invalid username: found '{escape(username)}', expected '{escape(user.username)}'"
            )
        with transaction.atomic():
            user.delete()
    elif name == "activity":
        ...
        if not path:
            return HttpResponseBadRequest("empty path")
        query = request.POST.get("query")
        if not query:
            return HttpResponseBadRequest("empty query")
        if action == "add":
            coder.bookmarks[path] = query
        elif action == "remove":
            coder.bookmarks.pop(path, None)
        else:
            return HttpResponseBadRequest(f"unknown action = {escape(action)}")
        if len(coder.bookmarks) > django_settings.MAX_BOOKMARKS_COUNT:
            return HttpResponseBadRequest("too many bookmarks")
        coder.save(update_fields=["bookmarks"])
    else:
...
def search(request, **kwargs):
    ...
    if query == "themes":
        ...
        qs = qs.annotate(
            disabled=Case(
                When(status=TeamStatus.NEW, then=Value(False)), default=Value(True), output_field=BooleanField()
            )
        ...
        if request.user.is_authenticated:
            qs = qs.annotate(
                my_weight=Case(
                    When(pk=request.user.coder.pk, then=Value(0)), default=Value(1), output_field=IntegerField()
                )
            )
        ...
        qs = Account.objects
        if contests := (request.get_filtered_list("contest") or request.get_filtered_list("contests[]")):
            qs = qs.filter(statistics__contest_id__in=contests)
            order = ["statistics__place_as_int", "-statistics__solving"]
            with_resource = False
        elif resources := (request.get_filtered_list("resource") or request.get_filtered_list("resources[]")):
            qs = qs.filter(resource_id__in=resources)
            order = ["resource_rank"]
            with_resource = False
        else:
        ...
        for division, statistics in divisions.items():
            if statistics:
                ...
                for s in statistics:
                    ...
                    s["score"] = 4.0 * (solving + upsolving) / max_total + 1.0 * solving / max_solving
